...
(function($, async) {
  ...
  if (currentPage == PAGE_INVENTORY) {
          ...
          if (numberOfQueuedItems > 0) {
            ...
            $("#inventory_sell_buttons").append(
              '<div id="inventory_items_spinner">' +
                spinnerBlock +
                '<div style="text-align:center">Processing ' +
                numberOfQueuedItems +
                " items</div>" +
                "</div>"
            );
        ...
        if (items.length == 0) {
          ...
          if (canSellSelectedItemsManually(items)) {
            ...
            $(".sell_manual > span").text(
              "Sell " +
                selectedItems +
                (selectedItems == 1 ? " Item Manual" : " Items Manual")
            );
          } else {
            $(".sell_manual").hide();
          }
      ...
      getInventorySelectedGemsItems(function(items) {
        ...
        if (items.length == 0) {
          ...
          $(".turn_into_gems > span").text(
            "Turn " +
              selectedItems +
              (selectedItems == 1 ? " Item Into Gems" : " Items Into Gems")
          );
        }
      });
    }
    ...
    function updateInventorySelection(item) {
      ...
      var selectedItemIdInventory =
        getActiveInventory().selectedItem.appid +
        "_" +
      ...
      var market_hash_name = getMarketHashName(
        getActiveInventory().selectedItem
      );
      ...
      market.getItemOrdersHistogram(item, false, function(err, histogram) {
        ...
        ownerActions.append(
          '<br/> <a class="btn_small btn_grey_white_innerfade" href="/market/listings/' +
            appid +
            "/" +
            market_hash_name +
            '"><span>View in Community Market</span></a>'
        );
        $(
          "#" +
            item_info_id +
        ...
        var itemId =
          getActiveInventory().selectedItem.assetid ||
          getActiveInventory().selectedItem.id;
    ...
    function updateInventoryUI(isOwnInventory) {
      ...
      loadAllInventories().then(
        function() {
          ...
          $("#inventory_pagecontrols").observe("childlist", "*", function(
            record
          ) {
            updateInventoryPrices();
          });
    ...
    function getInventoryItems() {
      ...
      for (var child in getActiveInventory().m_rgChildInventories) {
        for (var key in getActiveInventory().m_rgChildInventories[child]
          .m_rgAssets) {
          var value = getActiveInventory().m_rgChildInventories[child]
            .m_rgAssets[key];
  ...
  if (currentPage == PAGE_MARKET || currentPage == PAGE_MARKET_LISTING) {
      ...
      if (currentPage == PAGE_MARKET) {
        ...
        $("#tabContentsMyActiveMarketListingsRows > .market_listing_row").each(
          function() {
            ...
            unsafeWindow.g_rgAssets[appid][contextid][
              assetInfo.assetid
            ] = existingAsset;
            marketListingsQueue.push({
              listingid,
    ...
    function updateMarketSelectAllButton() {
      $(".market_listing_buttons").each(function() {
        ...
        $(".select_all > span", selectionGroup).text(
          invert ? "Deselect all" : "Select all"
        );
      });
    }
    ...
    function initializeMarketUI() {
      ...
      $(".relist_selected").on("click", "*", function() {
        ...
        for (var i = 0; i < marketList.matchingItems.length; i++) {
          if (
            $(marketList.matchingItems[i].elm).hasClass("overpriced") &&
            $(".market_select_item", $(marketList.matchingItems[i].elm)).prop(
              "checked"
            )
          ) {
    ...
    function getTradeOfferInventoryItems() {
      ...
      for (var child in getActiveInventory().rgChildInventories) {
        for (var key in getActiveInventory().rgChildInventories[child]
          .rgInventory) {
          var value = getActiveInventory().rgChildInventories[child]
            .rgInventory[key];
          if (typeof value === "object") {
            // Merges the description in the normal object, this is done to keep the layout consistent with the market page, which is also flattened.
            Object.assign(value, value.description);
  ...
  function openSettings() {
    var price_options = $(
      '<div id="price_options">' +
        ...
        'Calculate prices as the:&nbsp;<select class="price_option_input" style="background-color: black;color: white;border: transparent;" id="' +
        ...
        (getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 1
          ? 'selected="selected"'
          : "") +
        ">Maximum of the average history and lowest sell listing</option>" +
        '<option value="2" ' +
        (getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 2
          ? 'selected="selected"'
          : "") +
        ">Lowest sell listing</option>" +
        '<option value="3" ' +
        (getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 3
          ? 'selected="selected"'
          : "") +
        ">Highest current buy order or lowest sell listing</option>" +
        "</select>" +
        "<br/>" +
        "</div>" +
        '<div style="margin-bottom:6px;">' +
        'Hours to use for the average history calculated price:&nbsp;<input class="price_option_input" style="background-color: black;color: white;border: transparent;" type="number" step="2" id="' +
        ...
        "</div>" +
        '<div style="margin-bottom:6px;">' +
        'The value to add to the calculated price (minimum and maximum are respected):&nbsp;<input class="price_option_input price_option_price" style="background-color: black;color: white;border: transparent;" type="number" step="0.01" id="' +
        ...
        'Use the second lowest sell listing when the lowest sell listing has a low quantity:&nbsp;<input class="price_option_input" style="background-color: black;color: white;border: transparent;" type="checkbox" id="' +
        ...
        "</div>" +
        '<div style="margin-top:6px;">' +
        'Don\'t check market listings with prices of and below:&nbsp;<input class="price_option_input price_option_price" style="background-color: black;color: white;border: transparent;" type="number" step="0.01" id="' +
        ...
        'Show price labels in inventory:&nbsp;<input class="price_option_input" style="background-color: black;color: white;border: transparent;" type="checkbox" id="' +
        ...
        "</div>" +
        '<div style="margin-top:6px">' +
        'Show price labels in trade offers:&nbsp;<input class="price_option_input" style="background-color: black;color: white;border: transparent;" type="checkbox" id="' +
        ...
        '<div style="margin-top:24px">' +
        '<div style="margin-bottom:6px;">' +
        'Minimum:&nbsp;<input class="price_option_input price_option_price" style="background-color: black;color: white;border: transparent;" type="number" step="0.01" id="' +
        SETTING_MIN_NORMAL_PRICE +
        '" value=' +
        getSettingWithDefault(SETTING_MIN_NORMAL_PRICE) +
        ">&nbsp;" +
        'and maximum:&nbsp;<input class="price_option_input price_option_price" style="background-color: black;color: white;border: transparent;" type="number" step="0.01" id="' +
        ...
        'Market items per page:&nbsp;<input class="price_option_input price_option_price" style="background-color: black;color: white;border: transparent;" type="number" step="0.01" id="' +
        ...
        "<br/>" +
        '<div style="margin-top:6px;">' +
        'Automatically relist overpriced market listings (slow on large inventories):&nbsp;<input id="' +
        SETTING_RELIST_AUTOMATICALLY +
    ...
    var dialog = unsafeWindow
      .ShowConfirmDialog("Steam Economy Enhancer", price_options)
      .done(function() {
        setSetting(
          SETTING_MIN_NORMAL_PRICE,
